...
public:
    ...
    Point3 prevP;           // Previous vertex interaction point

    Point3 sampledNewP;     // sample() may set a new interaction point for current vertex
};
...
public:
    ...
    virtual Point3 evalInteractionPoint(const BSDFSamplingRecord &bRec, EMeasure measure = ESolidAngle) const { return{0,0,0}; }
    virtual bool updatesInteractionPoint(const BSDFSamplingRecord &bRec, EMeasure measure) const { return false; }
    virtual bool updatesInteractionPoint() const { return false; }
