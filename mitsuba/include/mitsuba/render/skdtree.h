...
struct TriangleAndNeighbours {
    const Point u1,u2,u3;
    const Normal n,nn1,nn2,nn3;
};
...
class MTS_EXPORT_RENDER ShapeKDTree : public SAHKDTree3D<ShapeKDTree> {
    ...
    using TrisT = std::map<unsigned, TriangleAndNeighbours>;

private:
    void fillPrimitives(const KDNode* __restrict currNode, TrisT &shapes, const Point &p, const Float r) const;
...
public:
    ...
    inline auto getPrimitivesInSphere(const Point &p, const Float r) const {
        TrisT shapes;
        fillPrimitives(m_nodes,shapes,p,r);
        return shapes;
    }
