...
static inline bool findTriMeshNeighbour(const TriMesh *mesh, const Point& p1, const Point &p2, ShapeKDTree::IndexType currIdx, ShapeKDTree::IndexType& neighbour) {
    const Triangle *triangles = mesh->getTriangles();
    const Point *positions = mesh->getVertexPositions();
    for (ShapeKDTree::IndexType j=0; j<mesh->getTriangleCount(); ++j) {
        if (j==currIdx) continue;
        const Triangle &tri = triangles[j];
        if ((positions[tri.idx[0]]==p1 && positions[tri.idx[1]]==p2) ||
            (positions[tri.idx[0]]==p1 && positions[tri.idx[2]]==p2) ||
            (positions[tri.idx[1]]==p1 && positions[tri.idx[0]]==p2) ||
            (positions[tri.idx[1]]==p1 && positions[tri.idx[2]]==p2) ||
            (positions[tri.idx[2]]==p1 && positions[tri.idx[0]]==p2) ||
            (positions[tri.idx[2]]==p1 && positions[tri.idx[1]]==p2)) {
            neighbour = j;
            return true;
        }
    }
    neighbour = (ShapeKDTree::IndexType)-1;
    return false;
}
    ...
    for (IndexType i=0; i<m_shapes.size(); ++i) {
        ...
        if (m_triangleFlag[i]) {
            ...
            for (IndexType j=0; j<mesh->getTriangleCount(); ++j) {
                const Triangle &tri = triangles[j];
                const Point &v1 = positions[tri.idx[0]];
                const Point &v2 = positions[tri.idx[1]];
                const Point &v3 = positions[tri.idx[2]];
                m_triAccel[idx].load(v1, v2, v3);
                m_triAccel[idx].shapeIndex = i;
                m_triAccel[idx].primIndex = j;

                // Find neighbours
                findTriMeshNeighbour(mesh, v2, v3, j, m_triAccel[idx].primNeighbourIndex[0]);
                findTriMeshNeighbour(mesh, v1, v3, j, m_triAccel[idx].primNeighbourIndex[1]);
                findTriMeshNeighbour(mesh, v1, v2, j, m_triAccel[idx].primNeighbourIndex[2]);
...
static inline Normal computeFaceNormal(const Point &v0, const Point &v1, const Point &v2) {
    Vector sideA(v1-v0), sideB(v2-v0);
    Normal n = cross(sideA, sideB);
...
void ShapeKDTree::fillPrimitives(const KDNode* __restrict currNode, TrisT &shapes, const Point &p, const Float r) const {
    if (!currNode)
        return;

    if (EXPECT_TAKEN(!currNode->isLeaf())) {
        const Float splitVal = (Float) currNode->getSplit();
        const int axis = currNode->getAxis();

        if (p[axis]+r>splitVal)
            fillPrimitives(currNode->getRight(),shapes,p,r);
        if (p[axis]-r<splitVal)
            fillPrimitives(currNode->getLeft(),shapes,p,r);
        return;
    }

    for (auto entry=currNode->getPrimStart(),
        last = currNode->getPrimEnd(); entry != last; entry++) {
        const auto idx = getIndices()[entry];

        const TriAccel &ta = m_triAccel[idx];
        if (EXPECT_TAKEN(m_triAccel[idx].k != KNoTriangleFlag)) {
            const auto tidx = ta.primIndex;
            auto it = shapes.lower_bound(tidx);
            if (it!=shapes.end() && it->first==tidx)
                continue;

            const auto shapeIndex = ta.shapeIndex;
            const auto* mesh = static_cast<const TriMesh *>(m_shapes[shapeIndex]);
            const auto* tris = mesh->getTriangles();
            const auto* positions = mesh->getVertexPositions();

            const auto tn1idx = ta.primNeighbourIndex[0];
            const auto tn2idx = ta.primNeighbourIndex[1];
            const auto tn3idx = ta.primNeighbourIndex[2];

            const Normal n   = computeFaceNormal(positions[tris[tidx].idx[0]],
                                                 positions[tris[tidx].idx[1]],
                                                 positions[tris[tidx].idx[2]]);
            const Normal n1  = tn1idx!=(IndexType)-1 ? 
                computeFaceNormal(positions[tris[tn1idx].idx[0]],
                                  positions[tris[tn1idx].idx[1]],
                                  positions[tris[tn1idx].idx[2]]) : 
                Normal{ 0,0,0 };
            const Normal n2  = tn2idx!=(IndexType)-1 ? 
                computeFaceNormal(positions[tris[tn2idx].idx[0]],
                                  positions[tris[tn2idx].idx[1]],
                                  positions[tris[tn2idx].idx[2]]) : 
                Normal{ 0,0,0 };
            const Normal n3  = tn3idx!=(IndexType)-1 ? 
                computeFaceNormal(positions[tris[tn3idx].idx[0]],
                                  positions[tris[tn3idx].idx[1]],
                                  positions[tris[tn3idx].idx[2]]) : 
                Normal{ 0,0,0 };

            shapes.emplace_hint(it,
                tidx, TriangleAndNeighbours{ 
                    positions[tris[tidx].idx[0]],
                    positions[tris[tidx].idx[1]],
                    positions[tris[tidx].idx[2]],
                    n, n1, n2, n3 
                });
