        ...
        for (size_t i=0; i<m_hilbertCurve.getPointCount(); ++i) {
            ...
            for (size_t j = 0; j<m_sampler->getSampleCount(); j++) {
                ...
                Path::alternatingRandomWalkFromPixel(m_scene, m_sampler,
                    ...
                    sensorDepth, offset, m_config.rrDepth, m_config.intensityDistribution, m_pool);
    ...
    void evaluate(BDPTWorkResult *wr,
        ...
        for (int s = (int) emitterSubpath.vertexCount()-1; s >= 0; --s) {
            ...
            for (int t = maxT; t >= minT; --t) {
                PathVertex
                    // *vsPredPred = emitterSubpath.vertexOrNull(s-2),
                    // *vtPredPred = sensorSubpath.vertexOrNull(t-2),
                ...
                if (vs->isEmitterSupernode()) {
                    ...
                    value = radianceWeights[t] *
                        vs->eval(scene, vsPred, vt, EImportance, m_config.intensityDistribution) *
                        vt->eval(scene, vtPred, vs, ERadiance, m_config.intensityDistribution);
                    ...
                    if (s == 1) {
                        ...
                        value = radianceWeights[t] * vt->sampleDirect(scene, m_sampler,
                            &tempEndpoint, &tempEdge, &tempSample, m_config.intensityDistribution, EImportance);
                        if (value.isZero())
                            continue;
                        vs = &tempSample; vsPred = &tempEndpoint; vsEdge = &tempEdge;
                        value *= vt->eval(scene, vtPred, vs, ERadiance, m_config.intensityDistribution);
                        vt->measure = EArea;
                    } else {
                        if (vs->isDegenerate())
                            continue;
                        ...
                        value = importanceWeights[s] * vs->sampleDirect(scene, m_sampler,
                            &tempEndpoint, &tempEdge, &tempSample, m_config.intensityDistribution, ERadiance);
                        if (value.isZero())
                            continue;
                        vt = &tempSample; vtPred = &tempEndpoint; vtEdge = &tempEdge;
                        value *= vs->eval(scene, vsPred, vt, EImportance, m_config.intensityDistribution);
                        vs->measure = EArea;
                    }
