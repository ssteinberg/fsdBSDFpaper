...
public:
    BDPTIntegrator(const Properties &props) : Integrator(props) {
        ...
        m_config.intensityDistribution = props.getBoolean("intensityDistribution", false);
        m_config.sampleDirect = props.getBoolean("sampleDirect", true);
        m_config.showWeighted = props.getBoolean("showWeighted", false);

        if (m_config.intensityDistribution)
            m_config.lightImage = false;

        if (props.hasProperty("wavelengths")) {
            Spectrum::m_wavelengths = props.getSpectrum("wavelengths");
        }
