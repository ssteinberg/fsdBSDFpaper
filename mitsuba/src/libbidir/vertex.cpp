...
static thread_local ref<BSDF> intensityDistributionBSDF = nullptr;
inline BSDF* createIntensityBSDF(const PathVertex *pred, const PathVertex *succ, ETransportMode mode, bool intensityDistribution) {
    if (intensityDistribution && (
        (mode == ERadiance && !!pred && pred->getType()==PathVertex::ESensorSample) ||
        (mode == EImportance && !!succ && succ->getType()==PathVertex::ESensorSample)
        )) {
        if (!intensityDistributionBSDF) {
            Properties props;
            props.setPluginName("diffuse");
            props.setSpectrum("diffuseReflectance", Spectrum{ 1.f });
            auto diffuse = static_cast<BSDF *> (PluginManager::getInstance()->
                        createObject(MTS_CLASS(BSDF), props));
            diffuse->configure();

            Properties tprops;
            tprops.setPluginName("twosided");
            intensityDistributionBSDF = static_cast<BSDF *> (PluginManager::getInstance()->
                        createObject(MTS_CLASS(BSDF), tprops));
            intensityDistributionBSDF->addChild(diffuse);
            intensityDistributionBSDF->configure();
        }
        return intensityDistributionBSDF.get();
    }
    return nullptr;
}
...
bool PathVertex::sampleNext(const Scene *scene, Sampler *sampler,
        ...
        bool intensityDistribution, ETransportMode mode, 
    ...
    switch (type) {
        ...
        case ESurfaceInteraction: {
                ...
                const BSDF* intensityBSDF = createIntensityBSDF(pred,succ,mode,intensityDistribution);
                if (!!intensityBSDF)
                    bsdf = intensityBSDF;
                ...
                if (bsdf->updatesInteractionPoint(bRec, measure) && bRec.sampledNewP != its.p) {
                    const auto tempd = pred->getPosition() - bRec.sampledNewP;
                    const auto lend = tempd.length();
                    wi = tempd/lend;

                    // Test visibility
                    const auto ray1 = Ray(pred->getPosition() - wi*Epsilon, -wi, 0, lend, its.time);
                    if (scene->rayIntersect(ray1))
                        return false;

                    // Update current vertex position
                    getIntersection().p = bRec.sampledNewP;
                    bRec.wi = its.toLocal(wi);
                    // Update previous vertex
                    // if (predpred && pred->update(scene, predpred, this, mode, pred->measure))
                        // return false;
                }
...
Spectrum PathVertex::eval(const Scene *scene, const PathVertex *pred,
        const PathVertex *succ, ETransportMode mode, bool intensityDistribution, EMeasure measure) const {
    ...
    switch (type) {
        ...
        case ESurfaceInteraction: {
                ...
                if (bsdf->updatesInteractionPoint(bRec, measure)) {
                    static thread_local ref<Sampler> sampler;
                    if (!bRec.sampler) {
                        if (!sampler) {
                        ...
                        bRec.sampler = sampler.get();
                    }

                    const auto p = bsdf->evalInteractionPoint(bRec, measure);
                    if (p != its.p) {
                        const auto temppd = predP - p;
                        const auto lenpd = temppd.length();
                        const auto tempsd = succP - p;
                        const auto lensd = tempsd.length();
                        if (lenpd<=0 || lensd<=0)
                            return Spectrum(0.0f);

                        const_cast<Intersection*>(&its)->p = p;
                        wi = temppd/lenpd;
                        wo = tempsd/lensd;
                        bRec.wi = its.toLocal(wi);
...
Spectrum PathVertex::sampleDirect(const Scene *scene, Sampler *sampler,
        PathVertex *endpoint, PathEdge *edge, PathVertex *sample, bool intensityDistribution, ETransportMode mode) const {
    ...
    if (isSurfaceInteraction()) {
        ...
        if (!emitter) {
            const BSDF *bsdf = its.getBSDF();
            if (bsdf->updatesInteractionPoint())
                return Spectrum(.0f);
        }
